...
const mocks = vi.hoisted(() => ({
  ...
  requirePortalPublicTenant: vi.fn(),
...
vi.mock('@/lib/customer-portal/public-tenant', () => ({
  getPortalPublicTenant: mocks.getPortalPublicTenant,
  requirePortalPublicTenant: mocks.requirePortalPublicTenant,
}))
...
vi.mock('@/lib/customer-portal/recovery-actions', () => ({
...
type MetadataGenerator = (input: {
  params: Promise<{ tenantSlug: string }>
}) => Promise<{ title?: string; robots?: unknown; referrer?: string }>

describe('public recovery routes', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.getPortalPublicTenant.mockResolvedValue({ tenantName: 'FreshCut' })
    mocks.requirePortalPublicTenant.mockImplementation(
      async (params: Promise<{ tenantSlug: string }>) => {
        const { tenantSlug } = await params
        const tenant = await mocks.getPortalPublicTenant(tenantSlug)
        if (!tenant) return mocks.notFound()
        return { tenantSlug, tenant }
      },
    )
    mocks.getRecoveryStateAction.mockResolvedValue({
      state: 'sent',
      attemptsRemaining: 5,
  ...
  it('is noindex/no-cache, no-store and renders only public tenant identity in the recovery shell', async () => {
    const html = renderToStaticMarkup(
      await RecoveryPage({ params: Promise.resolve({ tenantSlug: 'freshcut' }) }),
    )
  ...
  it('turns only the neutral session flag into the canonical CP-REC-11 toast', async () => {
    ...
    const arbitrary = renderToStaticMarkup(
      await RecoveryPage({
        params: Promise.resolve({ tenantSlug: 'freshcut' }),
  ...
  it('loads only neutral challenge state for verification and never channel/provider state', async () => {
    const html = renderToStaticMarkup(
      await VerifyPage({ params: Promise.resolve({ tenantSlug: 'freshcut' }) }),
    )
    expect(mocks.noStore).toHaveBeenCalled()
  ...
  it('resolves exact tenant-aware metadata on both recovery routes', async () => {
    const recoveryMetadata = (RecoveryRoute as unknown as { generateMetadata?: MetadataGenerator })
      .generateMetadata
    const verifyMetadata = (VerifyRoute as unknown as { generateMetadata?: MetadataGenerator })
      .generateMetadata
    ...
    await expect(
      recoveryMetadata({ params: Promise.resolve({ tenantSlug: 'freshcut' }) }),
    ...
    await expect(
      verifyMetadata({ params: Promise.resolve({ tenantSlug: 'freshcut' }) }),
    ).resolves.toMatchObject({
  ...
  it.each(['Bad-Slug', 'missing'])(
    'fails closed for invalid or unknown tenant %s',
    async (tenantSlug) => {
      mocks.getPortalPublicTenant.mockResolvedValue(null)
      await expect(RecoveryPage({ params: Promise.resolve({ tenantSlug }) })).rejects.toThrow(
        'NEXT_NOT_FOUND',
      )
      await expect(VerifyPage({ params: Promise.resolve({ tenantSlug }) })).rejects.toThrow(
        'NEXT_NOT_FOUND',
      )
    },
  )
