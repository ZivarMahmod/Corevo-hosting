...
export default async function CustomerPortalHistoryPage() {
  ...
  if (history.outcome !== 'ok') {
    return <PortalShell active="history" customerName={session.snapshot.customerName} tenantName={session.snapshot.tenantName} tenantSlug={session.snapshot.tenantSlug}><h1>Historik</h1><PortalErrorState variant="fetch-history" headingLevel="h3" /></PortalShell>
  }

  return (
    <PortalShell active="history" customerName={session.snapshot.customerName} tenantName={session.snapshot.tenantName} tenantSlug={session.snapshot.tenantSlug}>
