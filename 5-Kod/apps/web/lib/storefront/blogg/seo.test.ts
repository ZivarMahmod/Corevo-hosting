...
vi.mock('@/lib/storefront/blogg/load-blogg-post', () => ({
  loadBlogPostBySlug: vi.fn(async () => mocks.post),
}))

vi.mock('@/lib/storefront/blogg/load-blogg', () => ({
  loadPublishedBlogSitemapRows: vi.fn(async () => mocks.sitemapRows),
}))
...
vi.mock('@/lib/url', () => ({
  requestOrigin: vi.fn(async () => 'https://ateljen.corevo.se'),
}))
...
import { generateMetadata } from '@/app/(public)/blogg/[slug]/page'
import sitemap from '@/app/(public)/sitemap'
import { loadPublishedBlogSitemapRows } from './load-blogg'
...
describe('Goal 90 blogg SEO', () => {
  ...
  it('uses the post truth for canonical, description and open graph image', async () => {
    ...
    expect(metadata).toMatchObject({
      ...
      description: mocks.post.excerpt,
      alternates: { canonical: `/blogg/${mocks.post.slug}` },
      openGraph: {
        type: 'article',
        url: `/blogg/${mocks.post.slug}`,
        publishedTime: mocks.post.publishedAt,
        images: [{ url: mocks.post.coverImageUrl, alt: mocks.post.coverImageAlt }],
      },
    })
  })

  it('adds published posts for live blogg and never loads them for paused blogg', async () => {
    const live = await sitemap()
    expect(live).toContainEqual(expect.objectContaining({
      url: `https://ateljen.corevo.se/blogg/${mocks.post.slug}`,
      lastModified: mocks.post.publishedAt,
    }))
