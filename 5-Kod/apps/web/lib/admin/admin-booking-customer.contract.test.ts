...
describe('adminbokningens kundkoppling', () => {
  it('använder admin-RPC:n och skickar den valda kundraden eller den nya kundens uppgifter', () => {
    ...
    expect(action).toContain('queueBookingEvent')
    expect(action).toContain('notificationQueueMessage(notification)')
    expect(action).not.toMatch(/meddelande skickades/i)
