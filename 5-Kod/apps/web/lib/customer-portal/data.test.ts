...
const validBooking = {
  ...
  staffName: 'Sam',
  locationName: 'Corevo Studio',
  locationAddress: 'Testgatan 1',
  priceCents: 32900,
  currency: 'SEK',
}
...
describe('customer portal server DAL', () => {
  ...
  it('clamps list page size, forwards a stable cursor and exposes a next cursor', async () => {
    ...
    expect(first).toMatchObject({
      outcome: 'ok',
      scope: 'history',
      nextCursor: { startAt: validBooking.startAt, id: bookingId },
    })
  ...
  it('keeps an unknown runtime status but maps it to neutral presentation with no action rights', async () => {
    rpc.mockResolvedValue({
      data: {
        ...
        scope: 'history',
        pageSize: 20,
        items: [{ ...validBooking, status: 'awaiting_review' }],
      },
      error: null,
    })
    ...
    expect(result).toMatchObject({
      outcome: 'ok',
      items: [{ runtimeStatus: 'awaiting_review', presentationStatus: 'unknown' }],
    })
    if (result.outcome === 'ok') expect(result.items[0]).not.toHaveProperty('canCancel')
  })
  ...
  it('fails closed for invalid scope/cursor and malformed list projections', async () => {
    ...
    await expect(listPortalBookings({
      scope: 'history',
      cursor: { startAt: 'not-a-date', id: bookingId },
    })).resolves.toEqual({ outcome: 'unavailable' })
    expect(rpc).not.toHaveBeenCalled()

    rpc.mockResolvedValue({ data: { outcome: 'ok', scope: 'history', pageSize: 20, items: [{ id: 'bad' }] }, error: null })
    await expect(listPortalBookings({ scope: 'history' })).resolves.toEqual({ outcome: 'unavailable' })
    ...
    rpc.mockResolvedValue({
      data: {
        ...
        scope: 'history',
        pageSize: 20,
        items: [{ ...validBooking, startAt: '2026-02-30T10:00:00.000Z' }],
      },
      error: null,
    })
    await expect(listPortalBookings({ scope: 'history' })).resolves.toEqual({ outcome: 'unavailable' })
  ...
  it('fails closed if the detail RPC returns a different booking than requested', async () => {
