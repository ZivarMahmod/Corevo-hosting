...
describe('platform outbox summary correction', () => {
  ...
  it('keeps 0112 in the database release inventory through migration 0122', () => {
    const workflow = readFileSync(ciPath, 'utf8')
    expect(workflow.match(/--expected-latest 0122/g)).toHaveLength(2)
    expect(workflow.match(/--required-test-versions .*0113,0114,0115,0116,0117,0118,0119,0120,0121,0122/g)).toHaveLength(2)
  })
})
