...
import { getTenantFromHost, isPreviewHost, getSuperadminHost, getPlatformHost } from '@/lib/tenant'
import {
...
import { isRejectedPartnerIdentity, resolvePlatformIdentity } from '@/lib/auth/platform-identity'
...
export async function signIn(_prev: SignInState, formData: FormData): Promise<SignInState> {
  ...
  // Each account establishes a session only on its own host: platform operators
  // use superbooking, owner/staff use booking, and customers use their tenant host.
  // A credential used on the wrong door is signed out immediately. Preview/dev
  // stays unified for the local acceptance harness.
  const host = (await headers()).get('host')
  let loginHostKind: LoginHostKind | null = null
  if (!isPreviewHost(host)) {
    ...
    if (!access.allowed) {
      ...
      const rightHost =
        door === 'superadmin' ? getSuperadminHost() : door === 'platform' ? getPlatformHost() : null
      return {
        error: rightHost
  ...
  redirect(
    loginDestinationForHost({
      home: portalHomeFor({ roleLevel, platformAdmin, partnerAdmin }),
      next,
