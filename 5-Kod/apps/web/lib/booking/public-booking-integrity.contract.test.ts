...
const pinBookingMigration = fs.readFileSync(
  path.join(CODE_ROOT, 'supabase', 'migrations', '0118_pin_booking_verification.sql'),
  'utf8',
)
...
describe('public booking write integrity', () => {
  it('uses a storefront-only DB contract and keeps the admin contract separate', () => {
    ...
    expect(migration).toContain('grant execute on function public.create_storefront_booking')
    expect(migration).toContain('to service_role')
    expect(migration).toContain('revoke all on function public.create_public_booking')
    expect(migration).toContain('grant execute on function public.create_public_booking')
  ...
  it('creates named walk-ins through an atomic tenant-bound customer relation', () => {
    expect(staffWalkInMigration).toContain('create or replace function public.create_staff_walk_in')
    ...
    expect(staffWalkInMigration).toContain("v_customer, v_start, v_end, 'confirmed', v_price, null")
