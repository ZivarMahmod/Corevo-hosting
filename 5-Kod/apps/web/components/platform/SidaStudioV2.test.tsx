...
describe('SidaStudioV2 acceptance shell', () => {
  ...
  it('locks canonical geometry, shared theme tokens and mobile controls in CSS', () => {
    ...
    expect(css).toMatch(/\.embedded \.preview\s*\{[^}]*position:\s*sticky[^}]*top:\s*78px[^}]*overflow:\s*hidden/is)
    ...
    expect(component).toContain('className={styles.mobileSave}')
    expect(css).toMatch(/\.mobileActions[^}]*grid-template-columns:\s*1fr\s+1fr/is)
    expect(css).toMatch(/@media \(max-width: 767px\)[\s\S]*?\.mobileActions \{[^}]*position:\s*sticky/is)
    expect(css).not.toMatch(/@media \(max-width: 767px\)[\s\S]*?\.mobileActions \{[^}]*position:\s*fixed/is)
    ...
    expect(css).toMatch(/@media \(max-width: 767px\)[\s\S]*?\.embedded \.panel \{[^}]*padding:\s*12px;/is)
    expect(css).toMatch(/@media \(max-width: 767px\)[\s\S]*?\.embedded \{[^}]*height:\s*auto;[^}]*overflow:\s*visible;/is)
    expect(css).toMatch(/@media \(max-width: 767px\)[\s\S]*?\.embedded \.panel \{ overflow:\s*visible; \}/is)
    expect(css).toMatch(/@media \(max-width: 767px\)[\s\S]*?\.preview \{[^}]*box-sizing:\s*border-box;/is)
    expect(css).not.toMatch(/@media \(max-width: 767px\)[\s\S]*?\.preview \{[^}]*padding-bottom:\s*142px/is)
