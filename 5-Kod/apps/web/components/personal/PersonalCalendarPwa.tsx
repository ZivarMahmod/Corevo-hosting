...
import { BOOKING_STATUS_LABELS } from '@/lib/booking/confirmation-status'
...
export function PersonalCalendarPwa({
  ...
  useEffect(() => {
    ...
    ;((sheet && getFocusableElements(sheet)[0]) || sheet)?.focus()
    return () => {
    ...
    <section className={styles.calendarScreen} data-accept="personal-calendar">
      <header className={styles.calendarHeader}>
        <div>
          <h1>{heading}</h1>
          <p>
            {ownCalendar ? 'MINA BOKNINGAR' : 'BOKNINGAR'} · {bookings.length}{' '}
            {todayHref ? 'DEN HÄR DAGEN' : 'IDAG'}
          </p>
        </div>
        <div className={styles.dayControls}>
          <Link
            href={`/personal?dag=${previousDay}&personal=${selectedStaffId}`}
            aria-label="Föregående dag"
          >
            ‹
          </Link>
          <Link
            href={`/personal?dag=${nextDay}&personal=${selectedStaffId}`}
      ...
      {canViewAllCalendars && staff.length > 1 ? (
        <div className={styles.staffChips} aria-label="Kalender">
          {staff.map((member) => (
            ...
            >
              <span>{member.label.slice(0, 1).toUpperCase()}</span>
              {member.mine ? 'Jag' : member.label}
            </Link>
          ))}
        </div>
      ) : null}
      ...
      <div className={styles.timelineScroll}>
        <div className={styles.timeline} style={{ height: (endHour - startHour) * HOUR_PX }}>
          <div className={styles.times}>
            {hours.map((hour) => (
              <span key={hour} style={{ top: (hour - startHour) * HOUR_PX - 7 }}>
          ...
          <div className={styles.grid}>
            {bookings.map((booking) => {
              ...
              return (
                ...
                >
                  <strong>
                    {fmtTime(booking.startTs, booking.timeZone)} · {booking.customerLabel}
                  </strong>
        ...
        {bookings.length === 0 ? (
          <p className={styles.noBookings}>Inga bokningar den här dagen.</p>
        ) : null}
      </div>
        ...
        <div className={styles.sheetLayer} data-accept="booking-sheet">
          ...
          >
            ...
            <div className={styles.sheetMeta}>
              <span>
                <i />
                {ownCalendar ? 'din bokning' : 'bokning'} · {day}
              </span>
              <button type="button" aria-label="Stäng bokning" onClick={() => setSelectedId(null)}>
                ✕
              </button>
            </div>
            <div className={styles.sheetTitle}>
              <strong>{fmtTime(selected.startTs, selected.timeZone)}</strong>
              <span>{selected.serviceName ?? 'Bokning'}</span>
            </div>
            <div className={styles.sheetCustomer}>
              {selected.customerId ? (
                <ClientCard
                  ...
                  label={selected.customerLabel}
                  bookingNote={selected.customerNote}
                />
              ) : (
            ...
            {ownCalendar && (selected.status === 'pending' || selected.status === 'confirmed') ? (
              ...
              <p className={styles.sheetState}>
                Status: {BOOKING_STATUS_LABELS[selected.status] ?? 'Okänd status'}
              </p>
            )}
          </section>
        </div>
