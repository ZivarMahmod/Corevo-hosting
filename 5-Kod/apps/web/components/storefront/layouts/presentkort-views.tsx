...
function Empty({ purchaseClosed }: { purchaseClosed: boolean }) {
  return (
    <p role="status" className={styles.notice}>
      {purchaseClosed ? 'Onlineköp av presentkort är inte öppet just nu.' : 'Det finns inga valbara belopp just nu.'}
    </p>
  )
}
...
export function AuroraPresentkort({ config, purchaseClosed, tenantName }: ThemePresentkortViewProps) {
  const gift = useGiftState({ config, purchaseClosed, tenantName })
  return (
    <section className={styles.section} data-gift-theme="aurora" style={{ maxWidth: 960, margin: '0 auto', padding: '72px 28px 40px' }}>
      <div style={{ textAlign: 'center', marginBottom: 48 }}>
        <p style={{ margin: '0 0 10px', color: '#9B4D3C', fontSize: 13, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase' }}>Presentkort</p>
        ...
        <p style={{ margin: '0 auto', maxWidth: 440, color: '#765E54', fontSize: 16, lineHeight: 1.7 }}>Giltigt ett år, för allt i studion — buketter, kurser och bröllop.</p>
      </div>
      {gift ? <div className={grid} style={{ gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }}>
        <div style={{ background: '#F3DED4', padding: 20 }}><div style={{ border: '1px solid #9B4D3C', padding: '48px 32px', textAlign: 'center' }}>
          <p style={{ margin: '0 0 6px', fontFamily: "'Lora'", fontStyle: 'italic', fontSize: 34, color: '#8A3E2E' }}>{tenantName}</p>
          <p style={{ margin: '0 0 26px', fontSize: 11.5, letterSpacing: '0.24em', textTransform: 'uppercase', color: '#765E54' }}>presentkort</p>
          <p style={{ margin: 0, fontFamily: "'Lora'", fontWeight: 500, fontSize: 44, color: '#3A2A24' }}>{gift.amountLabel}</p>
          <p style={{ margin: '20px 0 0', fontFamily: "'Lora'", fontStyle: 'italic', fontSize: 14, color: '#9B4D3C' }}>gäller i tolv månader</p>
        </div></div>
        <div>{purchaseClosed ? <Empty purchaseClosed /> : <>
          <p style={{ margin: '0 0 14px', fontWeight: 700, fontSize: 13 }}>Välj belopp</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 28 }}><AmountButtons gift={gift} className={styles.aurora} style={(on) => ({ cursor: 'pointer', border: `1px solid ${on ? '#9B4D3C' : '#EAD8CD'}`, background: on ? '#9B4D3C' : '#fff', color: on ? '#fff' : '#3A2A24', padding: '13px 24px', fontWeight: 700, fontSize: 15 })} /></div>
          <p style={{ margin: '0 0 14px', fontWeight: 700, fontSize: 13 }}>Hur ska det levereras?</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 32 }}>{gift.modes.map((mode) => { const on = gift.mode === mode; return <button key={mode} type="button" className={button(styles.aurora)} aria-pressed={on} onClick={() => gift.pickMode(mode)} style={{ cursor: 'pointer', border: `1px solid ${on ? '#9B4D3C' : '#EAD8CD'}`, background: on ? '#9B4D3C' : '#fff', color: on ? '#fff' : '#3A2A24', padding: '13px 24px', fontWeight: 700, fontSize: 14 }}>{GIFT_DELIVERY_LABELS[mode]}</button> })}</div>
          <button type="button" className={button(styles.auroraBuy)} onClick={gift.add} style={{ border: 'none', cursor: 'pointer', background: '#9B4D3C', color: '#fff', fontWeight: 700, fontSize: 15, padding: '16px 36px' }}>{gift.added ? 'I korgen ✓' : `Lägg i korgen — ${gift.amountLabel}`}</button>
        </>}</div>
      </div> : <Empty purchaseClosed={purchaseClosed} />}
    </section>
  )
}

export function BlomstertorgetPresentkort({ config, purchaseClosed, tenantName }: ThemePresentkortViewProps) {
  const gift = useGiftState({ config, purchaseClosed, tenantName })
  ...
  return <section className={styles.section} data-gift-theme="blomstertorget" style={{ maxWidth: 720, margin: '0 auto', padding: '36px 28px 0' }}>
    ...
    </div></div>{purchaseClosed ? <Empty purchaseClosed /> : <>
    ...
    </>}</> : <Empty purchaseClosed={purchaseClosed} />}
  </section>
}

export function CalytrixPresentkort({ config, purchaseClosed, tenantName }: ThemePresentkortViewProps) {
  const gift = useGiftState({ config, purchaseClosed, tenantName })
...
export function EloriaPresentkort({ config, purchaseClosed, tenantName }: ThemePresentkortViewProps) {
  const gift = useGiftState({ config, purchaseClosed, tenantName })
