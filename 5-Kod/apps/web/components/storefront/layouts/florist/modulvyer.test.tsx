...
const WITH_VIEWS = THEME_SUITES.florist
  .map((theme) => ({ theme, views: themeModuleViews(theme.key) }))
  .filter(({ views }) => views.shop && views.blogg)

describe('florist-svitens modul-vyer', () => {
  it('VARJE mall i sviten äger både butiks- och bloggvyn', () => {
    expect(WITH_VIEWS).toHaveLength(THEME_SUITES.florist.length)
  })
  ...
  it.each(['calytrix', 'solsalt', 'sivsav', 'lunaria'])(
    ...
    (key) => {
      const theme = THEME_SUITES.florist.find((candidate) => candidate.key === key)!
      const Shop = themeModuleViews(key).shop!
      const content = {
        ...theme.content,
...
describe.each(WITH_VIEWS.map(({ theme, views }) => [theme.key, theme, views] as const))(
  'modul-vy: %s',
  (_key, theme, views) => {
    ...
    const shopHtml = (data: ShopData = DATA) =>
    ...
    it('priceFrom → priset skrivs "fr. " (formatProductPrice, aldrig mallens egen formatering)', () => {
      ...
      expect(shopHtml(from)).toContain('fr. 399')
      // …och utan flaggan står priset naket.
      expect(shopHtml()).not.toContain('fr. 399')
    })
