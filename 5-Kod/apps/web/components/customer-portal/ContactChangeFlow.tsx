...
import { maskBookingContact, normalizeBookingContact } from '@/lib/booking/contact-normalization'
import type { PortalContactChangeAction, PortalVerifiedContact } from '@/lib/customer-portal/types'
...
const CONSEQUENCE =
  'Dina andra inloggade enheter loggas ut och dina PIN-fria bokningsenheter återkallas.'
...
export function ContactChangeFlow({
  ...
  async function start() {
    ...
    if (result.outcome === 'max_attempts') {
      ...
      setError(
        `För många försök. Begär en ny kod om ${Math.ceil(result.retryAfterSeconds / 60)} min.`,
      )
      return
    }
  ...
  async function verifyCurrent(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!/^\d{6}$/.test(code)) {
      setError('Fel kod. Du har 5 försök kvar.')
      return
    }
    ...
    if (result.outcome === 'step_up_expired') {
      setStep('expired')
      return
    }
    if (result.outcome === 'expired') {
  ...
  return (
    <div className="cp-contact-change-layer">
      ...
      >
        ...
        <h2 id="contact-change-title" tabIndex={-1} ref={titleRef}>
          {stepTitle(step)}
        </h2>

        {step === 1 && (
          <>
            <p id="contact-change-description">
              Vi skickar en kod till din nuvarande kontakt {currentContact.maskedDestination}.
            </p>
            <p id="contact-change-why">
              Av säkerhetsskäl bekräftar du först din nuvarande kontaktuppgift och sedan den nya.
            </p>
            {error && (
              <p className="cp-contact-change-error" role="alert">
                {error}
              </p>
            )}
            <div className="cp-contact-change-actions">
              <button
                ...
                type="button"
                disabled={pinLocked}
                aria-disabled={pending || pinLocked || undefined}
                onClick={start}
              >
              ...
              <button
                ...
                onClick={() => !pendingRef.current && setStep('help')}
              >
        ...
        {step === 'help' && (
          <>
            <p>
              Av säkerhetsskäl kan du inte byta kontaktuppgift själv utan kod till din nuvarande
              kontakt. Kontakta {tenantName} för manuell kontroll.
            </p>
            <div className="cp-contact-change-actions">
              <a className="cp-btn cp-btn-primary" href={support.href}>
                {support.label}
              </a>
              <button className="cp-btn" type="button" onClick={() => setStep(1)}>
                Tillbaka
              </button>
            </div>
          </>
        ...
        {step === 2 && (
          <form className="cp-stack" noValidate onSubmit={verifyCurrent}>
            {deliveryState !== 'failed' && (
              <p className="cp-contact-change-channel" aria-live="polite">
                {deliveryState === 'resend_sent' ? (
                  'En ny kod har skickats.'
                ) : (
                  <>
                    Vi har skickat en kod via {currentSent.channel === 'sms' ? 'SMS' : 'e-post'}{' '}
                    till {currentSent.maskedDestination}
                  </>
                )}
              </p>
            )}
            <label className="cp-field" htmlFor="contact-current-pin">
              <span>Engångskod</span>
              <input
                id="contact-current-pin"
                className="cp-mono cp-code-input"
                type="text"
                inputMode="numeric"
                pattern="[0-9]*"
            ...
            {!resendBlocked && (
              <button
                ...
                aria-disabled={pending || cooldown > 0 || undefined}
                aria-live="polite"
                onClick={() => resend('current')}
              >
                {cooldown > 0
                  ? `Skicka ny kod om [00:${String(cooldown).padStart(2, '0')}]`
                  : 'Skicka ny kod'}
              </button>
            )}
          </form>
        ...
        {step === 3 && (
          <form className="cp-stack" noValidate onSubmit={sendNew}>
            {action !== 'change_email' && (
              <label className="cp-field" htmlFor="contact-country">
                <span>Landskod</span>
                <select id="contact-country" value="+46" disabled>
                  <option>+46</option>
                </select>
              </label>
            )}
            <label className="cp-field" htmlFor="contact-destination">
              ...
              <input
                ...
                name="destination"
                type={action === 'change_email' ? 'email' : 'tel'}
                inputMode={action === 'change_email' ? 'email' : 'tel'}
                autoComplete={action === 'change_email' ? 'email' : 'tel'}
                value={destination}
                disabled={pending}
                ref={inputRef}
        ...
        {step === 4 && newContact && (
          <form className="cp-stack" noValidate onSubmit={finalize}>
            {deliveryState !== 'failed' && (
              <p className="cp-contact-change-channel" aria-live="polite">
                {deliveryState === 'resend_sent' ? (
                  'En ny kod har skickats.'
                  ...
                  <>
                    Vi har skickat en kod via {newContact.channel === 'sms' ? 'SMS' : 'e-post'} till{' '}
                    {newContact.maskedDestination}
                  </>
                )}
              </p>
            )}
            <label className="cp-field" htmlFor="contact-new-pin">
              <span>Engångskod</span>
              <input
                id="contact-new-pin"
                className="cp-mono cp-code-input"
        ...
        {step === 'expired' && (
          <button className="cp-btn cp-btn-primary" type="button" onClick={restart}>
            Börja om
          </button>
        )}
        ...
        {step === 5 && (
          <>
            <p className="cp-contact-change-success" role="status">
              ...
              {action === 'change_phone'
                ? 'Telefonnumret är ändrat.'
                : action === 'add_phone'
                  ? 'Mobilnumret är tillagt.'
                  : 'Kontaktuppgiften är bytt.'}
            </p>
            <p className="cp-contact-change-consequence">{CONSEQUENCE}</p>
            <button className="cp-btn cp-btn-primary" type="button" onClick={closeReceipt}>
              Stäng
            </button>
          </>
        )}
        ...
        {typeof step === 'number' && step <= 4 && (
          <button
            className="cp-btn cp-contact-change-cancel"
            type="button"
            aria-disabled={pending || undefined}
            onClick={close}
          >
