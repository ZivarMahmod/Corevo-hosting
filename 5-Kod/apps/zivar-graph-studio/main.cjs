...
const { StudioV1Client } = require("./studio-v1-client.cjs");
...
const studioV1 = new StudioV1Client();
...
function writeState(input) {
  ...
  const state = {
    ...
    ui: input && typeof input === "object" ? input.ui : null,
    context: input && typeof input === "object" ? input.context : null,
    playback: input && typeof input === "object" ? input.playback : null,
...
function registerIpc() {
  safeHandle("studio:bootstrap", async () => {
    ...
    return {
      ...
      mcp: { name: "zivar-graph-studio-v1", url: "http://127.0.0.1:8868/mcp" },
  ...
  safeHandle("studio:v1", ({ operation, payload }) => studioV1.call(operation, payload));
