...
class GraphCanvas {
  ...
  targetId(target, project) {
    ...
    return this.nodes.find((node) => (
      ...
      && (!node.project || node.project === (project === "B" ? "B" : "A"))
    ))?.id || null;
  }
...
function renderInspector() {
  ...
  $("#node-path").textContent = detail.source_file || detail.sourceFile || detail.community_name || "";
  const metrics = [
...
function renderActivity() {
  ...
  for (const event of visible) {
    ...
    if (label) {
      latestTargets.set(event.agent || "Agent", {
        label,
        project: state.projects[event.project]?.name || event.project || "",
      });
    }
  }
  ...
  $("#agent-events").innerHTML = events.slice(0, 60).map((event) => {
    const metadata = [
      ...
      event.file || event.target?.sourceFile || "",
    ].filter(Boolean).join(" · ");
    return `
...
function openDetailTab(name) {
  $$("[data-detail-tab]").forEach((button) => button.classList.toggle("active", button.dataset.detailTab === name));
...
async function performSearch(query, autoOpen = false) {
  ...
  try {
    ...
    const needle = trimmed.toLocaleLowerCase("sv");
    const matches = comparisonMode
      ? (state.currentView?.nodes || []).filter((node) => (
        `${node.label || ""} ${node.sourceFile || ""}`.toLocaleLowerCase("sv").includes(needle)
      )).slice(0, 40)
      : await invoke("search", state.activeSlot, trimmed);
    if (autoOpen && matches[0]) {
      if (comparisonMode) {
        graph.focusTarget({ id: matches[0].id }, matches[0].project);
        await selectNode([matches[0].id]);
        results.hidden = true;
        return;
      }
      state.stacks[state.activeSlot].push({ mode: "neighborhood", key: matches[0].id, depth: 1 });
      await loadCurrentView();
      setTimeout(() => graph.focusTarget({ id: matches[0].id, sourceFile: matches[0].sourceFile, community: matches[0].community }, state.activeSlot), 80);
      return;
    }
    results.innerHTML = matches.map((match) => `
      <button data-search-id="${escapeHtml(match.id)}">
        <strong>${escapeHtml(match.label)}</strong>
        <small>${escapeHtml(match.sourceFile || match.communityName || match.kind)}</small>
      </button>
    `).join("");
...
function bindControls() {
  ...
  $$("[data-detail-tab]").forEach((button) => button.addEventListener("click", () => openDetailTab(button.dataset.detailTab)));
  $$("[data-mobile-panel]").forEach((button) => button.addEventListener("click", () => {
    setMobilePanel(state.mobilePanel === button.dataset.mobilePanel ? "" : button.dataset.mobilePanel);
  }));
  ...
  $("#search-results").addEventListener("click", async (event) => {
    ...
    if (["both", "diff"].includes(state.workspace)) {
      ...
      graph.focusTarget({ id }, id.startsWith("B:") ? "B" : "A");
      await selectNode([id]);
      $("#search-results").hidden = true;
      return;
    }
    state.stacks[state.activeSlot].push({ mode: "neighborhood", key: button.dataset.searchId, depth: 1 });
    $("#search-results").hidden = true;
    loadCurrentView();
  });
  ...
  $("#activity-live").addEventListener("click", () => setPlaybackMode("live"));
  $("#activity-play").addEventListener("click", () => setPlaybackMode("playing"));
  $("#activity-pause").addEventListener("click", () => setPlaybackMode("paused"));
