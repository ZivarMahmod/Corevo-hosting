...
test.describe('07 Kundadmin mobilchrome — source contract @readonly @contract', () => {
  ...
  test('07-C03 calendar swipes by day and moves the whole booking without a grip', () => {
    ...
    expect(component).toContain('calendarSwipeDirection')
    expect(component).toContain('SWIPE_THRESHOLD_PX = 48')
    ...
    expect(css).toMatch(/\.blockDrag\s*\{[\s\S]*?touch-action:\s*pan-y;/)
  ...
  test('07-C04 compact cards and unresolved visits preserve information without alarm styling', () => {
    ...
    expect(page).toContain('tone="info"')
    expect(page).toContain('Ingen driftstörning')
    expect(page).toContain('<details')
    expect(page).toContain('Visa bokningarna')
  })
