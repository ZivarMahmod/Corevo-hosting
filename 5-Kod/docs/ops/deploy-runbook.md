...
- **Production** — only on a `v*` **tag** or manual dispatch → production, behind the
  ...
  kräver därefter GitHub Environment-variabeln `PROD_DB_MIGRATION=0119` innan
...
`PROD_DB_MIGRATION=0119` i production-miljön **först efter verifierad DB-apply**.
**Secrets:**
...
| 7 | Prod-schema matchar Worker-koden | ✅ Prod verifierad till `0119`; checkpoint + `PROD_DB_MIGRATION=0119` krävs av workflowen. |
...
2. Ta backup-checkpoint, granska/applicera 0083–0088 på prod och verifiera
   `supabase_migrations.schema_migrations`; sätt därefter `PROD_DB_MIGRATION=0119`.
