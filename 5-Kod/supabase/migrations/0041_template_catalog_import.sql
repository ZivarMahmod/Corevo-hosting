-- ============================================================================
-- 0041 — Template-katalog import (option 2: fånga out-of-band-seed i migrations)
--
-- GOVERNANCE: mall-katalogen (26 mallar + ~230 template_slots) seedades out-of-band
-- via 4-Dokument-Underlag/03-template-katalog/templates-import.sql (genererad av
-- _import_templates.js) → fanns LIVE i prod men i NOLL migration → repo≠live-DB.
-- Denna fil = en VERBATIM-snapshot av den importen, promotad till migrations-historik
-- så en ren DB-rebuild reproducerar katalogen (repo==live). Källan/generatorn ligger
-- kvar i 4-Dokument-Underlag/ som regen-target.
--
-- REDAN APPLICERAD i prod (datan är live). Idempotent (on conflict do update) → en
-- re-apply är en no-op. SALVIA RÖRS EJ: templates-insertarna guardar `where key
-- <> 'salvia'` och importen seedar INGA salvia-slots → 0040:s kanoniska salvia-modell
-- bevaras. status=draft (active behålls om redan active).
-- ============================================================================
-- (original: genererad av _import_templates.js, READ-ONLY)
begin;
