-- goal-72 etapp 1b: PII-fri driftögonblicksbild för plattformsoperatören.
-- Tenantfiltret är en stabil seam för kommande partnerscope; dagens hårda
-- private.is_platform_admin()-grind gäller tills partnerrollen har egen DB-grind.
...
begin
  ...
  with queue as (
    select
      ...
      count(*) filter (
        where o.status = 'failed' and o.updated_at > now() - interval '24 hours'
  ...
  select
    ...
    coalesce(
      h.last_status = 'succeeded'
      and h.last_succeeded_at >= now() - interval '35 minutes'
