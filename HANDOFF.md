...
- Efter denna: **sekventiell körning på main, en goal i taget** (worktrees/kloner slopade). Ordning: G07→G06→G05→G08→G09→G10→G11. EN fil per mål: `2-Byggplan/goals/goal-NN-*.md` (klistra hela filen i Code). Roten = bara ej-klara mål; klara → `_klart/`. G09-prismodell BESLUTAD (se Spikade beslut: flöde 1 kund→salong application_fee=0; flöde 2 Corevo→salong = billing_model-val, manuell faktura, underlag i G08).
