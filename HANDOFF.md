...
- **NULÄGE-rad: G01–G12 KLAR (kod). G13 go-live PÅGÅR** — moln-DB konsoliderad till EN demo-salong (frisor1→`demo`, slug `demo`, host `demo.corevo.se`, customer-accounts PÅ, 3 konton kvar) + **frisor2 raderad**; `seed.sql` omskriven. Worker-bundle byggd från ASCII-kopia + deployad till `bokningsplatformen`. **KVAR (Zivar):** Custom Domains `booking`+`demo` på Workern, radera `frisor3` (owner-SQL, append-only-guard), JWT-hook. Se **`5-Kod/docs/ops/go-live-G13.md`**. ⚠️ **Bygg-bugg:** `ö` i mappnamnet `firsör-sas` kraschar OpenNext-bygget (esbuild edge-config ENOENT, INTE EPERM) → permanent fix = döp om mappen ASCII.
...
- **G10 säkerhet/compliance/drift (M9-ops): KOD-KLAR + verifierat** (2026-06-01). Migration **0008** (bookings.reminded_at + `private.rate_limit_hits` + `public.check_rate_limit` RPC) applicerad på molnet. Levererat:
  ...
  - **Notiser:** Resend-över-fetch (`lib/notifications/*`) — bekräftelse vid bokning/ombokning, avbokning, **kvitto** (webhook, gäst-mejl parsas ur note), **påminnelse-cron** (`/api/cron/reminders` bearer `CRON_SECRET` + `bookings.reminded_at`, filtrerar pending+confirmed). ⚠️ kräver `RESEND_API_KEY` + `NOTIFICATIONS_FROM` (Worker-secrets) + CF Cron Trigger.
  ...
  - **Verifierat NU:** build + lint + typecheck + **test (43)** gröna; RLS via MCP; headers via curl; secrets-skan. **OVERIFIERAD (kräver ops/secrets + deploy):** riktig e-postleverans (Resend), Sentry-leverans, GDPR auth-radering utan service-role-nyckel, headers på Workers-runtime, CF WAF/Cron-aktivering.
