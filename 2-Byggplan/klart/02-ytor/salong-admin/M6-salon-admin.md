...
- ⚠️ **Inga forcerade måste-fält.** Zivar hit friction: att lägga upp en tjänst tvingade fram required-värden. Formulär ska vara **optional-rika** — fyll det du vill, inget blockerar. Gäller tjänster + back-office-formulär generellt (samma princip som M7-onboardingen).
...
**Personal-onboarding (invite-flöde):**
- Lägg till personal sker i **salongens admin** — eller via **Zivars platform-admin (M7)** om salongen vill ha hjälp.
- Frisören skriver in sin **mejl → magic-link → bekräftar → skapar lösenord → inne**, med rätt roll/access tilldelad **direkt** (utan manuellt roll-pillande).
- ⚠️ Magic-link här = **engångs-invite**, inte löpande login. Löpande login = lösenord (jfr M4 §2.1). Ingen krock.
- Efter att frisören är inne sätter ägaren: **schema-baseline** (§3.5) + **vilka tjänster** frisören gör (`staff_services`).
- **Ta bort anställd** → access dras tillbaka rent.
- ⚠️ HANDOFF: invite-vägen är **kod-klar men overifierad** — kräver `SERVICE_ROLE_KEY` som Worker-secret. Verifiera i bygget.
...
- **Varje frisör har egen kadens.** Verifierat mot wavy: ett typiskt dag-raster är mest 30-min-steg (09:00, 09:30…) men med inströdda 15-min-tider (11:45, 12:45, 14:15, 14:45) — alltså ojämnt. Det ojämna i en "Alla / vem som helst"-vy = **unionen** av flera frisörers slots mergade. Misstolka inte det som *ett* schema. Per-frisör-kadensen sätts i M6; **union-vid-"Alla" är M3-logik.**
