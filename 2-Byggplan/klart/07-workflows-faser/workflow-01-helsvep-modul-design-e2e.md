...
- **Agent A — Publik storefront (M2):** ⚠️ bygg till referens-nivån i **`5-Kod/docs/design-referens-storefront.md`** (Tofifi/freshcut-klass — REDAKTIONELLT, inte platt: utility-bar, sticky nav, hero m. karusell, numrerade tjänster 01–05, om-sektion m. stats, plats + öppettider + karta, avslutande CTA, footer). Komplett salongssida m. riktiga tjänster/priser. **Freshcut-hemsida** som flaggskepp + **3 separata temamallar** (config nivå 1 + layout nivå 2 + scoped CSS nivå 3) = 3 DISTINKTA redaktionella stilar, inte platta varianter. Bevisa 2 tenants ser genuint olika ut.
- **Agent B — Bokningsmotor (M3, kärnan):** ⭐ INBÄDDAT i storefronten — "Boka tid" öppnar wizard IN-PAGE (inline/tab/drawer, samma nav+brand), kunden lämnar ALDRIG sidan. INGEN extern portal (Wavy = anti-mönstret). Se `design-referens-storefront.md`. Hela flödet klick för klick (tjänst→personal→dag→ledig tid→uppgifter→bekräftelse) i storefrontens designspråk, fina tidschips inte gråa rader. Alla states (laddar/inga tider/upptaget/lyckat/fel). Bekräftelse in-page med kvitto-känsla + kalender-krok.
